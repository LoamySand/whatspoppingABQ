...
-- Added for SCRUM-23 (Docker init): the source dump targets `public`,
-- but application code (db_utils.get_connection()) sets search_path to
-- `app`. This container-only init script creates that schema so the app
-- finds its tables. See SCRUM-7 for the real, upstream fix to the dump.
...
ALTER TABLE app.traffic_measurements OWNER TO postgres;
...
COMMENT ON TABLE app.traffic_measurements IS 'Traffic measurements collected around event venues';
...
COMMENT ON COLUMN app.traffic_measurements.measurement_time IS 'When traffic was measured (UTC)';
...
COMMENT ON COLUMN app.traffic_measurements.traffic_level IS 'Qualitative traffic assessment';
...
COMMENT ON COLUMN app.traffic_measurements.delay_minutes IS 'Delay compared to typical conditions';
...
COMMENT ON COLUMN app.traffic_measurements.raw_response IS 'Full API response for debugging';
...
COMMENT ON COLUMN app.traffic_measurements.event_id IS 'Event this traffic measurement is associated with (NULL for baseline traffic)';
...
COMMENT ON COLUMN app.traffic_measurements.baseline_type IS 'Type of baseline: weekly, monthly, seasonal, or NULL for event traffic';
...
CREATE VIEW app.api_usage_by_day AS
...
ALTER VIEW app.api_usage_by_day OWNER TO postgres;
...
COMMENT ON VIEW app.api_usage_by_day IS 'Daily API usage and cost tracking for last 30 days';
...
COMMENT ON TABLE app.events IS 'Stores event data scraped from various sources in Albuquerque';
...
COMMENT ON COLUMN app.events.event_id IS 'Auto-incrementing primary key';
...
COMMENT ON COLUMN app.events.event_name IS 'Name of the event';
...
COMMENT ON COLUMN app.events.venue_name IS 'Location where event takes place';
...
COMMENT ON COLUMN app.events.category IS 'Event category (festival, concert, sports, etc.)';
...
COMMENT ON COLUMN app.events.expected_attendance IS 'Estimated number of attendees';
...
COMMENT ON COLUMN app.events.latitude IS 'Venue latitude coordinate';
...
COMMENT ON COLUMN app.events.longitude IS 'Venue longitude coordinate';
...
COMMENT ON COLUMN app.events.source_url IS 'URL where event data was scraped from';
...
COMMENT ON COLUMN app.events.created_at IS 'Timestamp when record was first created';
...
COMMENT ON COLUMN app.events.updated_at IS 'Timestamp when record was last updated';
...
COMMENT ON COLUMN app.events.venue_id IS 'Foreign key to venue_locations table';
...
COMMENT ON COLUMN app.events.event_start_date IS 'First day of event';
...
COMMENT ON COLUMN app.events.event_end_date IS 'Last day of event (same as start_date for single-day)';
...
COMMENT ON COLUMN app.events.event_start_time IS 'Event start time';
...
COMMENT ON COLUMN app.events.event_end_time IS 'Event end time';
...
COMMENT ON COLUMN app.events.is_multi_day IS 'True if event spans multiple days';
...
COMMENT ON COLUMN app.events.sponsor IS 'Event sponsor/presenter';
...
COMMENT ON COLUMN app.events.cost_min IS 'Minimum ticket price';
...
COMMENT ON COLUMN app.events.cost_max IS 'Maximum ticket price';
...
COMMENT ON COLUMN app.events.cost_description IS 'Cost description (e.g., "Free", "$50-$100")';
...
COMMENT ON COLUMN app.events.phone IS 'Contact phone number';
...
COMMENT ON COLUMN app.events.email IS 'Contact email';
...
COMMENT ON COLUMN app.events.ticket_url IS 'URL to purchase tickets';
...
COMMENT ON COLUMN app.events.website_url IS 'Event or organizer website';
...
ALTER TABLE app.venue_locations OWNER TO postgres;
...
COMMENT ON TABLE app.venue_locations IS 'Geocoded venue locations with coordinates';
...
COMMENT ON COLUMN app.venue_locations.venue_id IS 'Auto-incrementing primary key';
...
COMMENT ON COLUMN app.venue_locations.venue_name IS 'Venue name (unique)';
...
COMMENT ON COLUMN app.venue_locations.latitude IS 'Latitude coordinate (WGS84)';
...
COMMENT ON COLUMN app.venue_locations.longitude IS 'Longitude coordinate (WGS84)';
...
COMMENT ON COLUMN app.venue_locations.place_id IS 'Google Places ID for API reference';
...
CREATE VIEW app.event_traffic_with_baseline AS
 SELECT e.event_id,
    e.event_name,
    e.event_start_date,
    e.event_start_time,
   ...
   FROM ((app.events e
     JOIN app.venue_locations v ON ((e.venue_id = v.venue_id)))
     LEFT JOIN app.traffic_measurements tm ON (((tm.event_id = e.event_id) OR ((tm.is_baseline = true) AND (tm.venue_id = v.venue_id)))))
...
ALTER VIEW app.event_traffic_with_baseline OWNER TO postgres;
...
COMMENT ON VIEW app.event_traffic_with_baseline IS 'Event traffic metrics compared to baseline traffic from same day-of-week and hour';
...
CREATE VIEW app.category_traffic_impact AS
 SELECT category,
   ...
   FROM app.event_traffic_with_baseline
  WHERE (event_measurements > 0)
  GROUP BY category
  ORDER BY (avg(impact_above_baseline)) DESC NULLS LAST;


ALTER VIEW app.category_traffic_impact OWNER TO postgres;
...
COMMENT ON VIEW app.category_traffic_impact IS 'Traffic impact statistics by event category';
...
CREATE VIEW app.collection_schedule_validation AS
 ...
 SELECT ue.event_id,
   ...
   FROM (upcoming_events ue
     LEFT JOIN app.traffic_measurements tm ON ((ue.event_id = tm.event_id)))
  GROUP BY ue.event_id, ue.event_name, ue.event_datetime
  ORDER BY ue.event_datetime;


ALTER VIEW app.collection_schedule_validation OWNER TO postgres;
...
COMMENT ON VIEW app.collection_schedule_validation IS 'Validates traffic collection for events happening soon';
...
CREATE VIEW app.collection_status_summary AS
 SELECT ( SELECT count(*) AS count
           FROM app.events) AS total_events,
    ( SELECT count(*) AS count
           FROM app.events
          WHERE (events.event_start_time IS NOT NULL)) AS events_with_time,
    ( SELECT count(*) AS count
           FROM app.venue_locations) AS total_venues,
    ( SELECT count(*) AS count
           FROM app.traffic_measurements) AS total_measurements,
    ( SELECT count(*) AS count
           FROM app.traffic_measurements
    ...
    ( SELECT max(traffic_measurements.measurement_time) AS max
           FROM app.traffic_measurements) AS last_measurement,
    ( SELECT max(traffic_measurements.measurement_time) AS max
           FROM app.traffic_measurements
...
ALTER VIEW app.collection_status_summary OWNER TO postgres;
...
COMMENT ON VIEW app.collection_status_summary IS 'High-level summary of data collection status';
...
CREATE VIEW app.data_quality_check AS
...
ALTER VIEW app.data_quality_check OWNER TO postgres;
...
COMMENT ON VIEW app.data_quality_check IS 'Identifies data quality issues across all tables';
...
CREATE VIEW app.event_impact_detail AS
 SELECT e.event_id,
    e.event_name,
    e.event_start_date,
    e.event_start_time,
   ...
   FROM ((app.events e
     JOIN app.venue_locations v ON ((e.venue_id = v.venue_id)))
     LEFT JOIN app.event_traffic_with_baseline etb ON ((e.event_id = etb.event_id)))
  WHERE (e.event_start_time IS NOT NULL)
  ORDER BY e.event_start_date DESC, e.event_start_time DESC;


ALTER VIEW app.event_impact_detail OWNER TO postgres;
...
COMMENT ON VIEW app.event_impact_detail IS 'Comprehensive event impact with baseline match quality indicators';
...
CREATE VIEW app.event_impact_summary AS
 SELECT event_id,
    event_name,
    event_start_date,
    event_start_time,
...
ALTER VIEW app.event_impact_summary OWNER TO postgres;
...
COMMENT ON VIEW app.event_impact_summary IS 'Simplified event impact view with classification levels';
...
CREATE VIEW app.event_traffic_data AS
 SELECT e.event_id,
    e.event_name,
    e.event_start_date,
    e.event_start_time,
   ...
   FROM ((app.events e
     JOIN app.venue_locations v ON ((e.venue_id = v.venue_id)))
     LEFT JOIN app.traffic_measurements tm ON ((v.venue_id = tm.venue_id)))
...
ALTER VIEW app.event_traffic_data OWNER TO postgres;
...
COMMENT ON VIEW app.event_traffic_data IS 'Events joined with traffic measurements within 2-hour window';
...
CREATE VIEW app.events_missing_traffic AS
 SELECT e.event_id,
    e.event_name,
    e.event_start_date,
    e.event_start_time,
  ...
  WHERE ((e.event_start_time IS NOT NULL) AND (NOT (EXISTS ( SELECT 1
           FROM app.traffic_measurements tm
          WHERE (tm.event_id = e.event_id)))))
  ORDER BY e.event_start_date, e.event_start_time;


ALTER VIEW app.events_missing_traffic OWNER TO postgres;
...
COMMENT ON VIEW app.events_missing_traffic IS 'Events that should have traffic data but don''t';
...
CREATE VIEW app.events_multi_day AS
 SELECT event_id,
    event_name,
    venue_name,
    event_start_date,
...
ALTER VIEW app.events_multi_day OWNER TO postgres;
...
COMMENT ON VIEW app.events_multi_day IS 'Events spanning multiple days';
...
CREATE VIEW app.events_single_day AS
 SELECT event_id,
    event_name,
    venue_name,
...
ALTER VIEW app.events_single_day OWNER TO postgres;
...
COMMENT ON VIEW app.events_single_day IS 'Events occurring on a single day';
...
CREATE VIEW app.recent_collection_activity AS
...
ALTER VIEW app.recent_collection_activity OWNER TO postgres;
...
COMMENT ON VIEW app.recent_collection_activity IS 'Hourly breakdown of collection activity in last 24 hours';
...
CREATE SEQUENCE app.traffic_measurements_measurement_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE app.traffic_measurements_measurement_id_seq OWNER TO postgres;
...
ALTER SEQUENCE app.traffic_measurements_measurement_id_seq OWNED BY app.traffic_measurements.measurement_id;
...
CREATE VIEW app.traffic_summary_by_venue AS
 SELECT v.venue_id,
    v.venue_name,
    ...
    min(tm.measurement_time) AS first_measurement,
    max(tm.measurement_time) AS last_measurement
   FROM (app.venue_locations v
     LEFT JOIN app.traffic_measurements tm ON ((v.venue_id = tm.venue_id)))
  GROUP BY v.venue_id, v.venue_name;


ALTER VIEW app.traffic_summary_by_venue OWNER TO postgres;
...
COMMENT ON VIEW app.traffic_summary_by_venue IS 'Summary statistics of traffic by venue';
...
CREATE VIEW app.venue_baseline_patterns AS
 SELECT v.venue_id,
    v.venue_name,
    tm.day_of_week,
        CASE tm.day_of_week
   ...
   FROM (app.venue_locations v
     JOIN app.traffic_measurements tm ON ((v.venue_id = tm.venue_id)))
...
ALTER VIEW app.venue_baseline_patterns OWNER TO postgres;
...
COMMENT ON VIEW app.venue_baseline_patterns IS 'Baseline traffic patterns by venue, day of week, and hour';
...
CREATE SEQUENCE app.venue_locations_venue_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE app.venue_locations_venue_id_seq OWNER TO postgres;
...
ALTER SEQUENCE app.venue_locations_venue_id_seq OWNED BY app.venue_locations.venue_id;
...
CREATE VIEW app.venues_missing_baseline AS
 SELECT v.venue_id,
    v.venue_name,
   ...
   FROM (app.venue_locations v
     LEFT JOIN app.traffic_measurements tm ON (((v.venue_id = tm.venue_id) AND (tm.is_baseline = true))))
  GROUP BY v.venue_id, v.venue_name
  ORDER BY (count(tm.measurement_id)), v.venue_name;


ALTER VIEW app.venues_missing_baseline OWNER TO postgres;
...
COMMENT ON VIEW app.venues_missing_baseline IS 'Venues with incomplete or outdated baseline data';
...
ALTER TABLE ONLY app.traffic_measurements ALTER COLUMN measurement_id SET DEFAULT nextval('app.traffic_measurements_measurement_id_seq'::regclass);
...
ALTER TABLE ONLY app.venue_locations ALTER COLUMN venue_id SET DEFAULT nextval('app.venue_locations_venue_id_seq'::regclass);
...
ALTER TABLE ONLY app.traffic_measurements
...
ALTER TABLE ONLY app.venue_locations
...
CREATE INDEX idx_category ON app.events USING btree (category);
...
CREATE INDEX idx_event_date_range ON app.events USING btree (event_start_date, event_end_date);
...
CREATE INDEX idx_event_end_date ON app.events USING btree (event_end_date);
...
CREATE INDEX idx_event_start_date ON app.events USING btree (event_start_date);
...
CREATE INDEX idx_events_venue ON app.events USING btree (venue_id);
...
CREATE INDEX idx_traffic_baseline_type ON app.traffic_measurements USING btree (baseline_type, day_of_week, hour_of_day);
...
CREATE INDEX idx_traffic_event ON app.traffic_measurements USING btree (event_id);
...
CREATE INDEX idx_traffic_level ON app.traffic_measurements USING btree (traffic_level);
...
CREATE INDEX idx_traffic_time ON app.traffic_measurements USING btree (measurement_time);
...
CREATE INDEX idx_traffic_venue ON app.traffic_measurements USING btree (venue_id);
...
CREATE INDEX idx_traffic_venue_time ON app.traffic_measurements USING btree (venue_id, measurement_time DESC);
...
CREATE INDEX idx_venue_coords ON app.venue_locations USING btree (latitude, longitude);
...
CREATE INDEX idx_venue_name ON app.events USING btree (venue_name);
...
ALTER TABLE ONLY app.events
    ADD CONSTRAINT events_venue_id_fkey FOREIGN KEY (venue_id) REFERENCES app.venue_locations(venue_id);
...
ALTER TABLE ONLY app.traffic_measurements
    ADD CONSTRAINT traffic_measurements_event_id_fkey FOREIGN KEY (event_id) REFERENCES app.events(event_id);
...
ALTER TABLE ONLY app.traffic_measurements
    ADD CONSTRAINT traffic_measurements_venue_id_fkey FOREIGN KEY (venue_id) REFERENCES app.venue_locations(venue_id) ON DELETE CASCADE;
...
GRANT SELECT ON TABLE app.traffic_measurements TO PUBLIC;
...
GRANT SELECT ON TABLE app.api_usage_by_day TO PUBLIC;
...
GRANT SELECT ON TABLE app.events TO PUBLIC;
...
GRANT SELECT ON TABLE app.venue_locations TO PUBLIC;
...
GRANT SELECT ON TABLE app.collection_schedule_validation TO PUBLIC;
...
GRANT SELECT ON TABLE app.collection_status_summary TO PUBLIC;
...
GRANT SELECT ON TABLE app.data_quality_check TO PUBLIC;
...
GRANT SELECT ON TABLE app.events_missing_traffic TO PUBLIC;
...
GRANT SELECT ON TABLE app.events_multi_day TO PUBLIC;
...
GRANT SELECT ON TABLE app.events_single_day TO PUBLIC;
...
GRANT SELECT ON TABLE app.recent_collection_activity TO PUBLIC;
...
GRANT SELECT ON TABLE app.traffic_summary_by_venue TO PUBLIC;
...
GRANT SELECT ON TABLE app.venue_baseline_patterns TO PUBLIC;
...
GRANT SELECT ON TABLE app.venues_missing_baseline TO PUBLIC;
