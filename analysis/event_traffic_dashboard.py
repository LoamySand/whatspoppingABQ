...
def load_event_data():
    ...
    numeric_cols = [
        'latitude', 'longitude', 'event_avg_delay', 'baseline_avg_delay',
        'impact_from_speed', 'event_avg_speed', 'baseline_avg_speed',
        'speed_difference_mph', 'impact_from_delay', 'speed_reduction_pct'
    ]
    ...
    if 'event_measurements' in df.columns:
        df['measurement_count'] = pd.to_numeric(
            df['event_measurements'], errors='coerce'
        ).fillna(0)
...
def load_category_data():
    ...
    numeric_cols = [
        'event_count', 'events_with_baseline', 'avg_impact_minutes',
        'avg_speed_reduction_pct', 'max_impact_minutes', 'avg_event_speed',
        'avg_baseline_speed', 'avg_speed_difference', 'pct_high_impact'
    ]
    for col in numeric_cols:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce')
...
QUALITY_ORDER = ['excellent', 'good', 'fair', 'poor', 'unreliable',
                 'no_baseline_data', 'no_event_data', 'unknown']
...
try:
    ...
    selected_qualities = st.sidebar.multiselect(
        ...
        default=[q for q in available_qualities
                 if q not in ('no_event_data', 'unreliable', 'unknown')],
        help="'Unreliable' = baseline had only one measurement."
    )

    st.sidebar.markdown("**Baseline Confidence**")
    available_confidence = [c for c in CONFIDENCE_ORDER
    ...
    with col1:
        st.metric(
            ...
            len(filtered_df[filtered_df['event_measurements'] > 0])
        )
    ...
    with col4:
        high_impact = len(reliable_events[
            reliable_events['speed_reduction_pct'] > THRESHOLDS['high']
        ])
        st.metric(
            "High Impact Events",
            high_impact,
    ...
    with col1:
        ...
        cat_with_data = filtered_df[
            ...
            pct_high_impact=('speed_reduction_pct', lambda x: round((x > 15).sum() / len(x) * 100, 1))
        ).reset_index()
        ...
        if len(cat_with_data) > 0:
            ...
            fig_category.add_hline(
                y=THRESHOLDS['moderate'], line_dash="dash", line_color="gold",
                annotation_text="Moderate (8%)"
            )
            fig_category.add_hline(
                y=THRESHOLDS['high'], line_dash="dash", line_color="orange",
                annotation_text="High (15%)"
            )
            fig_category.add_hline(
                y=THRESHOLDS['severe'], line_dash="dash", line_color="red",
    ...
    with conf_col1:
        conf_counts = filtered_df['baseline_confidence'].value_counts().reindex(
            CONFIDENCE_ORDER, fill_value=0
        ).reset_index()
    ...
    with conf_col2:
        conf_reduction = filtered_df[
            filtered_df['speed_reduction_pct'].notna()
        ].groupby('baseline_confidence')['speed_reduction_pct'].mean().reindex(
            CONFIDENCE_ORDER
        ).reset_index()
    ...
    if len(comparison_df) > 0:
        ...
        fig_comparison.add_trace(go.Scatter(
            ...
            hovertemplate=(
                ...
                'Speed reduction: ' +
                comparison_df['speed_reduction_pct'].round(1).astype(str) + '%<br>' +
                'Quality: ' + comparison_df['data_quality'].astype(str) +
                '<extra></extra>'
            )
        ))
    ...
    if len(filtered_df) > 0:
        ...
        top_events = rankable.nlargest(10, 'speed_reduction_pct')[[
            'event_name', 'venue_name', 'category', 'event_start_date',
            'speed_reduction_pct', 'impact_from_speed',
            'event_avg_speed', 'baseline_avg_speed',
            'impact_level', 'data_quality', 'baseline_confidence'
        ]].copy()

        top_events['event_start_date'] = pd.to_datetime(
            top_events['event_start_date']
        ).dt.strftime('%Y-%m-%d')
        ...
        try:
            st.dataframe(
                ...
                column_config={
                    ...
                    "Impact (min)":        st.column_config.NumberColumn(
                                               "Impact (min)", format="%.2f"),
