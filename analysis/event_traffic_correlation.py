    ...
    try:
        with conn.cursor() as cur:
            ...
            event_datetime = datetime.combine(
                event["event_start_date"], event["event_start_time"] or datetime.min.time()
            )
...
def analyze_event_impact(event: dict) -> dict:
    ...
    if not measurements:
        return {"has_data": False, "reason": "No traffic measurements available"}
    ...
    if before_stats and during_stats:
        if before_stats["avg_delay"] is not None and during_stats["avg_delay"] is not None:
            ...
            impact["delay_increase_pct"] = (
                ...
                / (abs(before_stats["avg_delay"]) + 1)
                * 100  # +1 to avoid division by zero
            )
...
def get_impact_summary(analyses: list[dict]) -> dict:
    ...
    category_avg = {
        cat: sum(delays) / len(delays) if delays else 0 for cat, delays in category_impacts.items()
    }

    # Top events by impact
    events_with_impact = [a for a in analyses if a["impact"].get("delay_increase") is not None]

    top_events = sorted(
        events_with_impact, key=lambda x: x["impact"]["delay_increase"], reverse=True
    )[:10]
...
if __name__ == "__main__":
    ...
    for level, count in sorted(
        summary["impact_levels"].items(), key=lambda x: impact_order.get(x[0], 999)
    ):
