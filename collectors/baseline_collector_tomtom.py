...
def collect_baseline_for_venue_tomtom(
    venue_id: int, venue_name: str, lat: float, lon: float, baseline_type: str = "weekly"
) -> list:
    """
    ...
    measurement = measure_traffic_tomtom(
        origin_lat=lat, origin_lng=lon, dest_lat=lat, dest_lng=lon, point_name=venue_name
    )
    ...
    if measurement:
        ...
        logger.info(" Collected 1 baseline measurement")
...
if __name__ == "__main__":
    ...
    measurements = collect_baseline_for_venue_tomtom(
        venue_id=1, venue_name="Isotopes Park", lat=35.0781, lon=-106.6044
    )
