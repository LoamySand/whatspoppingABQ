...
def get_traffic_flow_at_point(lat: float, lon: float, point_name: str = None) -> dict | None:
    """
    ...
    params = {"key": TOMTOM_API_KEY, "point": f"{lat},{lon}", "unit": "MPH"}
    ...
    try:
        ...
        logger.info(
            f" {point_name or 'Point'}: {traffic_level}, {current_speed:.1f} mph, delay {delay_minutes:.2f} min"
        )
...
def measure_traffic_tomtom(
    origin_lat: float, origin_lng: float, dest_lat: float, dest_lng: float, point_name: str = None
) -> dict | None:
    """
...
if __name__ == "__main__":
    ...
    measurement = get_traffic_flow_at_point(
        lat=35.188087, lon=-106.613448, point_name="Known congested area"
    )
