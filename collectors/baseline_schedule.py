...
def run_baseline_collection():
    ...
    log_file = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "baseline_collection_log.txt"
    )
    ...
    if not should_collect:
        if group is None:
            ...
            logger.info(
                f"ℹ Baseline collection week (Group {group}) - but not at a collection time"
            )
        ...
        return {"collected": False, "reason": "Not collection time"}

    logger.info(" Baseline collection time!")
...
if __name__ == "__main__":
    ...
    if group:
        print(" Currently in baseline collection week")
