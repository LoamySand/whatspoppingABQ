...
def scrape_events_task(max_pages: int = 3):
    ...
    if events:
        ...
        logger.info(
            f"  Start: {events[0].get('event_start_date')} {events[0].get('event_start_time')}"
        )
...
def validate_events_task(events: list):
    ...
    for event in events:
        ...
        if is_valid:
            ...
            invalid_events.append({"event": event.get("event_name"), "reason": message})
...
def generate_summary_task(load_stats: dict):
    ...
    print("Data Quality:")
    print(
        f"  Multi-day events: {stats['multi_day_events']} ({stats['multi_day_events']*100//stats['total_events'] if stats['total_events'] else 0}%)"
    )
    print(
        f"  Events with times: {stats['events_with_times']} ({stats['events_with_times']*100//stats['total_events'] if stats['total_events'] else 0}%)"
    )
    print(
        f"  Events with cost: {stats['events_with_cost']} ({stats['events_with_cost']*100//stats['total_events'] if stats['total_events'] else 0}%)"
    )
    print(f"  Free events: {stats['free_events']}")
    print(
        f"  Events with sponsors: {stats['events_with_sponsors']} ({stats['events_with_sponsors']*100//stats['total_events'] if stats['total_events'] else 0}%)"
    )
    print()
    ...
    if multi_day_events:
        ...
        for event in multi_day_events[:3]:
            ...
            print(
                f"    {event['event_start_date']} to {event['event_end_date']} ({event['duration_days']} days)"
            )
...
def handle_flow_failure(flow, flow_run, state):
    ...
    send_failure_alert(flow_name=flow.name, error=error, run_name=flow_run.name)
...
def handle_flow_crash(flow, flow_run, state):
    ...
    send_crash_alert(
        flow_name=flow.name, details=f"Flow run '{flow_run.name}' crashed unexpectedly."
    )
