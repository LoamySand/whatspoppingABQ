...
for i, event in enumerate(events, 1):
    ...
    for _j in range(2):
        meas_time, traffic_data = create_measurement(event, event_datetime, -1.0, "before", pattern)

        insert_traffic_measurement(
            venue_id=event["venue_id"], measurement_time=meas_time, traffic_data=traffic_data
        )
    ...
    for _j in range(2):
        meas_time, traffic_data = create_measurement(event, event_datetime, 1.0, "after", pattern)
