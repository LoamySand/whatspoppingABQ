...
from database.db_utils import get_event_statistics, insert_events
...
print(
    f"Events with times: {with_times} ({with_times*100//len(valid_events) if valid_events else 0}%)"
)
print(
    f"Events with cost info: {with_cost} ({with_cost*100//len(valid_events) if valid_events else 0}%)"
)
print(
    f"Events with sponsors: {with_sponsor} ({with_sponsor*100//len(valid_events) if valid_events else 0}%)"
)
